...
class Prx:
    ...
    context.set_alpn_protocols(["http/1.1","spdy/2",'TLS/1.2'])
    ...
    def data(self):
        ...
        try:
            ...
            if(type(t) is int):
                full = re.search(r'(?<=e-hentai.org)/fullimg(.*?)"',self.htmldata).group(0)
                print(full)
                fullimg = open("./fullimg.txt","rb").read()
                if(b'"https://e-hentai.org'+full.encode()+b"\r\n" in fullimg):
                    print("存在")
                else:
                    open("./fullimg.txt","ab").write(b'"https://e-hentai.org'+full.encode()+b"\r\n")
        except:
            pass
        '''
        '''
        ...
        self.fenk = da.split(" ")                        
        dio = re.sub(r"https://"+self.hostname+r".*?/",hts+self.__localhost+":8000/",da)
 ...
 文件名和文件数据
        '''
        if(self.sou_addr!=''):
            ip = socket.gethostbyname_ex(self.hostname)[2]
            try:
                if not ip[1]:
                    print("索引位置的值不存在")
                else:
                    s = socket.create_connection((ip[1],443),source_address=self.sou_addr)
                    
                    ss = self.context.wrap_socket(s,server_hostname=ip[1])
                ...
                s = socket.create_connection((ip[0],443),source_address=self.sou_addr)
        ...
        if(self.ipw!=None):
            ...
            print(ss.cipher())
            print(ss.selected_alpn_protocol())
            u1 = 'www.xvideos.com'
            ss.send(b"GET "+self.path.encode("utf-8")+b" HTTP/1.1\r\nHost: "+self.hostname.encode("utf-8")+b'''\r\nUser-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8\r\n\r\n''')
            self.sockets = ss
            dli = self.data()
            ...
            try:
                if not ip[1]:
                    ...
                    s = socket.create_connection((ip[1],443))
                    ss = self.context.wrap_socket(s,server_hostname=ip[1])
                    ss.send(b"GET "+self.path.encode("utf-8")+b" HTTP/1.1\r\nHost: "+self.hostname.encode("utf-8")+b"\r\nUser-agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Safari/537.36\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8\r\nConnection: keep-alive\r\n\r\n")
                    self.sockets = ss
                    dli = self.data()
