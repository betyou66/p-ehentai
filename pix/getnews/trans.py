...
def plot_attn_weights(attn_weights):
    ...
    for i in range(len(attn_weights)):
        nu.append(attn_weights[i].detach().cpu().numpy()) 
    def xh(x,a):
        strs = ""
        value = 0
        while x>value:
            #x = np.arange(0, 5, 0.1)
            a[value].plot(nu[value], color = 'red', marker = "o")
            value = value + 1
        return strs
    fig,axes = plt.subplots(nrows=len(nu),sharex='row')
    xh(len(nu),axes)  
    plt.title("loss")  
    #plt.xlabel("X")  
    #plt.ylabel("Y")  
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        """
        #函数实现矩阵乘法和除法Q*K/sqrt(d_k) sqrt(d_k)是为了防止Q*K过大，导致溢出
        #sqrt是平方根运算，d_k是K的维度
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        for layer in self.layers:
            ...
            # 创建输入
            #enc_inputs = torch.tensor([[1, 2, 3, 4, 5]])  # 示例输入
            # 前向传递
            # 绘制注意力权重
            #plot_attn_weights(enc_self_attns[0][0], enc_inputs.size(1), enc_inputs.size(0), 0)
...
class Transformer(nn.Module):
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        #print(enc_self_attns," enc_self_attns")
        # dec_outputs: [batch_size, tgt_len, d_model], dec_self_attns: [n_layers, batch_size, n_heads, tgt_len, tgt_len], dec_enc_attn: [n_layers, batch_size, tgt_len, src_len]
        dec_outputs, dec_self_attns, dec_enc_attns = self.decoder(
            dec_inputs, enc_inputs, enc_outputs)
        
        #print(dec_self_attns," dec_self_attns")
...
for epoch in range(epochs):
    for enc_inputs, dec_inputs, dec_outputs in loader:
        ...
        print(loss," loss")
        plot_attn_weights(outputs)
        print(dec_outputs.view(-1))
        plt.plot(dec_outputs.view(-1)[0].decath().cpu().numpy())
        plt.show()
