...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.directory = "C:/dow/stable-diffusion-webui/一些小脚本/pix/server"
        inps = [self.server]
        ...
        if(os.path.isfile("C:/dow/stable-diffusion-webui/一些小脚本/pix/server/"+sta.query+sta.types)):
            match sta.types:
                case ".html":
            ...
            if self.path == "/":
                ...
                try:
                    ...
                    if(we.find_all('span')==[]):
                        ...
                        #multiprocessing.Process(linkall.qs("pix/server/"+sta.query+sta.types,"pix/server/"+'a'+sta.query+sta.types)).start()
                    ...
                    print('except')
                    self.wfile.write(open("C:/dow/stable-diffusion-webui/一些小脚本/pix/server/"+sta.query+sta.types,"rb").read())
                    #linkall.stars("pix/server/"+sta.query+sta.types)
                    #linkall.stars("pix/server/"+self.query+self.types)
                    #self.request.close()
        if self.path == "/":
            match sta.types:
                case ".html":
            ...
            if(os.path.isfile("C:/dow/stable-diffusion-webui/一些小脚本/pix/server/index.html")):
...
#清除服务器缓存
def dele():
    for i in os.listdir('C:\dow\stable-diffusion-webui\一些小脚本\pix\server'):
        if(re.search(r'(.html)',i)==None):
            pass
        else:
            os.remove('C:\dow\stable-diffusion-webui\一些小脚本\pix\server/'+i)
dele()
